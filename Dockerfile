...
COPY --link --chmod=755 frankenphp/ensure-permissions.sh /usr/local/bin/ensure-permissions
...
FROM frankenphp_dev AS workspace

HEALTHCHECK NONE
COPY --link --chmod=755 frankenphp/workspace-entrypoint.sh /usr/local/bin/workspace-entrypoint
...
RUN <<-EOF
	...
	if [ -f importmap.php ]; then php bin/console asset-map:compile; fi
	chmod +x bin/console
	chmod -R g=u var
	sync
EOF
